...
async function cloudLoad() {
  if (!GS.piUser) return;
  try {
    const res = await fetch(`${SUPA_URL}/rest/v1/players?pi_uid=eq.${GS.piUser.uid}&select=*`, {
...
async function piAuth() {
  try {
    ...
    GS.piUser = auth.user;
    document.getElementById('piUserBadge').textContent = 'π ' + auth.user.username;
    document.getElementById('piUserBadge').style.display = 'block';
    await cloudLoad();
    showScreen('splash');
    setSplashBG();
  } catch(e) { playGuest(); }
...
function handleIncomplete(payment) {
  ...
  fetch(`${SUPA_URL}/rest/v1/payments`, {
    ...
    body: JSON.stringify({ payment_id: payment.identifier, status: 'incomplete', pi_uid: GS.piUser?.uid })
  }).catch(()=>{});
}
...
async function buy(item, amount) {
  if (!GS.piUser) { alert('Please login with Pi to purchase!'); return; }
  const actions = {
    ammo:   () => { curAmmo = Math.min(curAmmo + 10, 30); updAmmo(); },
    lives:  () => { GS.lives += 3; updHUD(); },
    speed:  () => { GS.speedBoost = true; },
    worlds: () => { GS.allWorlds = true; },
    coins:  () => { GS.coins += 1000; updHUD(); },
    mega:   () => { GS.allWorlds = true; GS.lives += 5; curAmmo = Math.min(curAmmo+30,30); updAmmo(); updHUD(); },
  };
  try {
    await Pi.createPayment(
      { amount, memo: 'Super Pio: ' + item, metadata: { item } },
      {
        onReadyForServerApproval: pid => {
          fetch(`${SUPA_URL}/rest/v1/payments`, {
            method:'POST',
            headers:{'Content-Type':'application/json','apikey':SUPA_KEY,'Authorization':`Bearer ${SUPA_KEY}`},
            body: JSON.stringify({ payment_id:pid, item, amount, pi_uid:GS.piUser.uid, status:'approved' })
          }).catch(()=>{});
        },
        onReadyForServerCompletion: (pid, _) => {
...
function showLeaderboard() {
  alert('🏆 Leaderboard coming soon!\n\nLogin with Pi to compete globally.');
}
